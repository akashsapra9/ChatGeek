...
async function startSlcServer(app) {
  ...
  const options = {
    key: fs.readFileSync(path.resolve(SLC_KEY)), cert: fs.readFileSync(path.resolve(SLC_CERT)),
  ...
  wss.on("connection", (ws, req) => {
    ...
    const clientCN = peer.subject.CN; if (!SLC_ALLOWED.includes(clientCN)) {
    ...
    ws.on("message", async (buf) => {
      try {
        ...
        switch (op) {
          case "SEND_CHAT": {
            // Expect the SLC envelope: msg.body should contain routing info and payload
            // Try multiple names to be tolerant: 'to', 'toUserId', 'recipient'
            const body = msg.body || {};
            const toUserId = body.to || body.toUserId || body.recipient || (body.payload && body.payload.to);
            const chatId = body.chatId || (body.payload && body.payload.chatId) || (body.payload && body.payload.chat_id);
            ...
            // Build payload to send to Finlay's network layer.
            // We'll include both the useful body and a trimmed envelope to preserve context.
            const payloadForNetwork = {
              // The actual message content (either plaintext or an object with ciphertext/signature)
              content: body.content ?? body.ciphertext ?? body.payload ?? {},
              // convenience fields
              chatId,
              // include the whole envelope for downstream use if they want it
              envelope: {
                from: msg.from,
                to: msg.to,
                op: msg.op,
