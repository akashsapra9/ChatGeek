...
(async () => {
  ...
  ws.on("open", async () => {
    ...
    // Signed advertise
    const adv = await makeSigned("USER_ADVERTISE", { user_id: "user_X", server_id: PEER_ID, meta: { display_name: "Test User" } });
    setTimeout(() => send(ws, adv), 150);

    // Signed remove
    const rem = await makeSigned("USER_REMOVE", { user_id: "user_X", server_id: PEER_ID });
