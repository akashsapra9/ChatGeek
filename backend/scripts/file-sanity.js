require("dotenv").config();
const WebSocket = require("ws");
const { signPayload } = require("../network/crypto/signing");

const PORT = parseInt(process.env.MESH_WS_PORT || "7081", 10);
const URL  = `ws://127.0.0.1:${PORT}`;
const PEER_ID   = process.env.DUMMY_PEER_ID || "peer_dummy";
const PEER_PUB  = "";
const PEER_PRIV = "";
const TARGET    = process.env.SOCP_DEV_LOCAL_USER || "LOCAL-USER-UUID"; // must be mapped to "local" for demo

if (!PEER_PUB || !PEER_PRIV) { console.error("Missing dummy peer keys."); process.exit(2); }

function send(ws, f){ ws.send(JSON.stringify(f)); }
const now = () => Date.now();

async function signed(type, payload) {
  const sig = await signPayload(payload, PEER_PRIV);
  return { type, from: PEER_ID, to: "*", ts: now(), payload, sig };
}

(async () => {
  const ws = new WebSocket(URL);
  ws.on("open", async () => {
    // Teach server our pubkey
    send(ws, { type:"SERVER_HELLO_LINK", from:PEER_ID, to:"*", ts:now(),
               payload:{ url:"ws://dummy", pubkey_b64url:PEER_PUB } });

    const start = await signed("FILE_START", { to_user: TARGET, file_id:"file-123", name:"demo.bin", size:3, sha256:"<sha256>" });
    setTimeout(()=> send(ws, start), 150);

    const c0 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:0, ciphertext:"<c0>" });
    const c1 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:1, ciphertext:"<c1>" });
    const c2 = await signed("FILE_CHUNK", { to_user: TARGET, file_id:"file-123", index:2, ciphertext:"<c2>" });
    setTimeout(()=> send(ws, c0), 280);
    setTimeout(()=> send(ws, c1), 360);
    setTimeout(()=> send(ws, c2), 440);

    const end = await signed("FILE_END", { to_user: TARGET, file_id:"file-123" });
    setTimeout(()=> send(ws, end), 520);

    setTimeout(()=> process.exit(0), 800);
  });
})();
